...
class ImageProcessJobTest {

    private fun createJob() = ImageProcessJob.create(
    ...
    fun `초기 상태는 PENDING이다`() {
        val request = createJob()
        assertEquals(JobStatus.PENDING, request.status)
    ...
    fun `startProcessing 호출 시 상태가 PROCESSING으로 변경된다`() {
        ...
        assertEquals(JobStatus.PROCESSING, request.status)
    ...
    fun `complete 호출 시 상태가 COMPLETED로 변경되고 resultUrl이 저장된다`() {
        ...
        assertEquals(JobStatus.COMPLETED, request.status)
    ...
    fun `fail 호출 시 상태가 FAILED로 변경된다`() {
        ...
        request.startProcessing("job-001")
        request.fail()
        assertEquals(JobStatus.FAILED, request.status)
